...
# 재활용 방법과 설명을 정의한 사전
recycle_methods = {
    "플라스틱": '🧴 모든 플라스틱은 깔끔하게 헹궈 줘. <br> 🧽💦 플라스틱이 "나도 깔끔한 게 좋아!"라고 말하고 있어. 라벨도 꼭 떼 줘야 해! 😄💕 플라스틱이 "내 라벨은 필요 없어!"라고 자랑스러워해.',  # 플라스틱에 대한 설명
...
selected_method = st.sidebar.selectbox("재활용품별 분리 방법 보기", list(recycle_methods.keys()))
if selected_method:
...
if file is None:
    ...
    st.image(image, caption='Uploaded Image.', use_column_width=True, width=0.5)
