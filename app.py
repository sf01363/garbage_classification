...
if file is None:
    ...
    if pred_probability >= threshold:
        st.success(f'분류 결과 : 해당 재활용품이 "{pred_class}"일 확률은 {pred_probability:.2f}입니다.', unsafe_allow_html=True)
        if pred_class in recycle_methods:
            st.write(f'{recycle_methods[pred_class]}', unsafe_allow_html=True)
        else:
            st.warning("해당하는 배출 방법이 없습니다.", unsafe_allow_html=True)
    else:
        st.warning(f'분류 결과 : 일반 쓰레기일 확률이 높습니다.', unsafe_allow_html=True)
